...
namespace Mogo.Util
{
    ...
    public partial class SystemConfig
    {
        ...
        {
            string cfgStr;
			//LoggerHelper.Debug ("LoadCfgInfo() CfgPath="+CfgPath);
            if (File.Exists(CfgPath))
            {
				//LoggerHelper.Debug ("file exist");
                cfgStr = Utils.LoadFile(CfgPath);
            }
            else
            {
				//LoggerHelper.Debug ("file not exist");
                var cfgUrl = Utils.LoadResource(Utils.GetFileNameWithoutExtention(CFG_FILE));
				//LoggerHelper.Debug ("cfgUrl="+cfgUrl);
                cfgStr = DownloadMgr.Instance.DownLoadText(cfgUrl);
            }
			//LoggerHelper.Debug ("cfgStr="+cfgStr);
