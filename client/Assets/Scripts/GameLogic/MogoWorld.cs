...
public class MogoWorld
{
    ...
    static public void OnEnterWorld(EntityParent entity)
    {
        ...
        {
            //Debug.LogError("Space has the same id:" + entity.ID);
            return;
        }
    ...
    {
		//Debug.Log ("OnBaseLogin() ip: " + ip + " port: " + port);
        ServerProxy.Instance.Disconnect();
        //Debug.Log("base ip: " + ip + " port: " + port);
    ...
    {
        //Debug.Log("OnEntityCellAttached " + info.position);
    ...
    static private void OnEntityAttached(BaseAttachedInfo baseInfo)
    {
        ...
        {
            //Debug.LogError("Entity Attach Error.");
            return;
        }
    ...
    static private void AOINewEntity(CellAttachedInfo info)
    {
        ...
        {
            //Debug.Log("has same id entity in world");
            return;
        }
        ...
        switch (info.entity.Name)
        {
            case "Avatar"://对应Avatar.def
                ...
                {
                    //Debug.Log("EntityPlayer over count: " + playerCount);
                    return;
                }
            ...
            case "Drop": //对应Drop.def
                ...
                //Debug.LogError("fuck end " + info.id + " n " + newPosi.x + " " + newPosi.y + " " + newPosi.z);
                break;
        ...
        if (!isLoadingScene)
            entity.CreateModel();
        //Debug.LogError("AOINewEntity: etype: " + info.entity.Name + " eid: " + entity.ID + " pos: " + entity.position);
        OnEnterWorld(entity);
    ...
    {
        //Debug.LogError("CreateDummy difficulty:" + difficulty);
    ...
    static public void ConnectServer(string ip, int port)
    {
        ...
        //Debug.Log("connnect rst: " + rst);
		Debug.Log("ConnectServer() connnect rst: " + rst);
    }
    ...
    {
        //Debug.Log("onConnected server. " + message);
        //if (nErrorID == 0)
        //{
    ...
    static public void Start()
    {
        ...
        {
            //Debug.LogError("Missing GlobalData!");
            return;
        }
    ...
    static public void Login()
    {
        ...
        if (serverInfo == null)
        {
            //Debug.LogError("Server Info index error: " + SystemConfig.Instance.SelectedServer);
        ...
        Action AfterGetInfo = () =>
        {
            ...
            //if (serverInfo.ip == "192.168.20.107")
        ...
        {
            //Debug.LogInfo("PlatformSdkManager.Instance.Login();");
#if UNITY_IPHONE
    ...
    {
		Debug.Log ("MogoWorld.SwitchScene("+orgSceneId+" => "+newSceneId);
        ...
        scenesManager.LoadInstance(newSceneId, (isLoadScene) =>
        {
            TimerHeap.AddTimer(0, 0, () =>
            {
                ...
                else
                {
                    ...
                    //Debug.LogError("************************* " + newSceneId + " " + canAutoFight);
                }
    ...
    {
        //Debug.LogInfo("progress: " + progress);
    ...
    private static void LoginReq(bool rst)
    {
        if (rst)
        {
            ...
            if (MogoWorld.rc)
            {
                //Debug.LogError("send rc key " + reConnectKey);
    ...
    static private void SceneLoaded(int sceneId, bool isInstance)
    {
        try
        {
            foreach (var item in entities)
            {
                ...
                //Debug.LogError("SceneLoaded:" + item.Value.ID);
        ...
        //Debug.Log("EventDispatcher.theRouter.Count: " + EventDispatcher.TheRouter.Count);
    }
    ...
    static public void LoadGears(int id, Action action)
    {
        try
        {
            ...
            foreach (var data in gearDataMap)
            {
                ...
                if (go == null)
                {
                    // Debug.Log("Name Path Wrong" + data.Value.gameObjectName);
                    continue;
                }
                ...
                {
                    // Debug.Log("Length Wrong: " + data.Value.gameObjectName + " name: " + argName.Length + " type: " + argType.Length + " arg: " + arg.Length);
                    continue;
                }
                ...
                foreach (var field in type.GetFields())
                {
                    ...
                    if (fieldType.IsArray)
                    {
                        ...
                        else if (elementType == typeof(GameObject))
                        {
                            ...
                            for (int j = 0; j < argString.Length; j++)
                            {
                                ...
                                if (argGo == null)
                                {
                                    // Debug.Log("arg name path wrong: " + data.Value.gameObjectName + " no " + field.Name + " no " + argString[j]);
                                    break;
                                }
                    ...
                    else
                    {
                        if (fieldType.IsSubclassOf(typeof(GearParent)))
                        {
                            ...
                            if (argGo == null)
                            {
                                // Debug.Log("arg name path wrong: : " + data.Value.gameObjectName + " no " + argsString);
                                break;
                            }
                        ...
                        {
                            //Debug.Log(component.name + " " + argsString + " " + go.name);
