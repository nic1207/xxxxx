...
namespace Mogo.Game
{
    public partial class EntityMyself : EntityPlayer
    {
            ...
            if (!ItemEquipmentData.dataMap.ContainsKey((int)(equip.templateId)))
            {
                switch (vocation)
                {
                    ...
                    case Vocation.Mage:
                        ...
                        break;
                    default:
                        Debug.LogError("vocation Error:" + vocation);
                        return subtype;
                }
            }
            else
        ...
        {
            base.OnEnterWorld();
			//Debug.Log("OnEnterWorld() Avatar name: " + name);
        ...
        public void CheckDmgBase()
        {
            ...
            {
                //Debug.LogError(curHp + "   " + def + "   " + atk + " d  " + DmgBase + "    " + deviator);
                UsedTool();
            }
        }
        ...
        private void TaskGuide()
        {
            ...
            {
                Debug.Log("PathPoint " + pathPoint + " Not Exist!");
            }
        }
        ...
        public void HandleLevelChangeCheck(byte value)
        {
            if (value != level)
            {
                ...
                {
					//Debug.Log("nextLevelExp="+(uint)AvatarLevelData.dataMap[(int)value].nextLevelExp);
                    nextLevelExp = (uint)AvatarLevelData.dataMap[(int)value].nextLevelExp;
                }
                else
                    Debug.Log("Can't Find nextLevelExp with this level: " + value);
        ...
        public int CalGoldMetallurgyLastUseTimes()
        {
            ...
            {
                Debug.LogError("PrivilegeData no contains key = " + MogoWorld.thePlayer.VipLevel);
                return 0;
            }

            if (m_VipRealStateMap == null)
            {
                //Debug.LogError("VipRealStateMap is null");
                return 0;
            }

            if (m_VipRealStateMap.ContainsKey((int)VipRealStateEnum.DAILY_GOLD_METALLURGY_TIMES) == false)
            {
                Debug.LogError("VipRealStateMap no contains key " + (int)VipRealStateEnum.DAILY_GOLD_METALLURGY_TIMES);
                return 0;
            }
        ...
        public int CaGoldMetallurgylDiamondCost(int times)
        {
            ...
            for (int i = 0; i < times; i++)
            {
                ...
                {
                    Debug.Log("PriceListData no contains key = " + GOLD_METALLURGY_ID);
                    return -1;
                }

                if (!PriceListData.dataMap[GOLD_METALLURGY_ID].priceList.ContainsKey(currentTimes))
                {
                    Debug.Log("PriceListData priceList no contains key = " + currentTimes);
                    return -1;
                }
        ...
        private void CheckTime(uint svrTime)
        {
            ...
            toolCnt++;
            Debug.LogError("may be used tool cnt: " + toolCnt + "c1 c2 " + send[0] + "," + send[1] + "c1` c2` " + recvClt[0] + "," + recvClt[1] + "s1 s2" + recv[0] + "," + recv[1]);
            recv.RemoveAt(0);
            send.RemoveAt(0);
            recvClt.RemoveAt(0);
            if (toolCnt >= 2)
