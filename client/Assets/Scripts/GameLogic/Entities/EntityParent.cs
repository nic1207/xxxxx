...
namespace Mogo.Game
{
    ...
    public partial class EntityParent : NotifyPropChanged, INotifyPropChanged
    {
            ...
            if (Transform)
            {
                ...
                else
                {
                    ...
                    if (myself != null)//������ײʧ�ܾ���������������
                    {
                        ...
                        Debug.LogWarning("Pull character to born point: " + map.enterX * 0.01 + ", " + map.enterY * 0.01);
                        Vector3 bornPoint;
        ...
        public void GotoPreparePosition()
        {
            if (Transform)
            {
                GameObject.layer = 14;//IgnoreColl
        ...
        public void Equip(int _equipId)
        {
            ...
            if (equip.mode > 0)
            {
                ...
                //    CreateActualModel();
                //    stopWatch.Stop();
                //    Debug.Log("CreateModel:" + stopWatch.Elapsed.Milliseconds);
        ...
        public void SynEntityAttrs(AttachedInfo info)
        {
            ...
            {
                //Debug.Log("SynEntityAttrs:------ " + prop.Key + " " + prop.Value);
        ...
        protected void SetAttr(EntityDefProperties propInfo, object value, Type type)
        {
            ...
            try
            {
                if (prop != null)
                {
                    // Debug.Log("prop: " + prop.Name + " value: " + value);
                ...
                else
                {
                    ...
                    //Debug.LogInfo("Static property not found: " + propInfo.Name);
                }
            }
            catch (Exception ex)
            {
                Debug.LogError("SetAttr error: " + propInfo.VType.VValueType + ":" + propInfo.Name + " " + value.GetType() + ":" + value + "\n" + ex);
                Debug.LogError("prop: " + prop + " this: " + this.GetType());
            }
        }
        ...
        protected void AddListener()
        {
            ...
            if (ety == null)
            {
                Debug.LogWarning("Entity not found: " + entityType);
                return;
            }
            foreach (var item in ety.ClientMethodsByName)
            {
				...
				if (method != null) {
					var e = new KeyValuePair<string, Action<object[]>> (String.Concat (Mogo.Util.Utils.RPC_HEAD, methodName), (args) => {//RPC�ص��¼�����
						try {
							//Debug.Log("RPC_resp: " + methodName);
							...
							Debug.LogError (String.Format ("RPC resp error: method name: {0}, message: {1} {2} {3}", methodName, sb.ToString (), inner.Message, inner.StackTrace));
						}
					});
					EventDispatcher.AddEventListener<object[]> (e.Key, e.Value);
					m_respMethods.Add (e);
				} else {
					//Debug.LogWarning ("Method not found: " + item.Key);
				}
            }
        }
        ...
        {
            //Debug.LogWarning("EventDispatcher.TheRouter.Count: " + EventDispatcher.TheRouter.Count);

            //Debug.LogWarning("m_respMethods: " + m_respMethods.Count);
        ...
        /// <summary>
        /// Gens the uniq message.
        /// </summary>
        /// <returns>The uniq message.</returns>
        ...
        /// <summary>
        /// Adds the callback in frames.
        /// </summary>
        /// <param name="callback">Callback.</param>
        /// <param name="inFrames">In frames.</param>
        ...
        public void HandleBuff(ushort buffId, byte isAdd, uint time)
        {
            if (buffManager == null)
            {
                Debug.Log("buffManager == null,obejct:" + Transform.gameObject.name + ",id:" + ID);
                return;
            }
