...
namespace Mogo.Game
{
    public partial class EntityMercenary
    {
        ...
        public override bool ProcCastSpell(int skillBagIndex, int reversal)
        {
            ...
            //Debug.LogError("skillBagIndex" + skillBagIndex + " "  + m_monsterData.skillIds.Count);
            if (m_monsterData.skillIds.Count < skillBagIndex || skillBagIndex <= 0)
            {
                return false;
            }
        ...
        public override bool ProcInSkillRange(int skillBagIndex)
        {
            ...
            else
            {
                ...
                if (distance >= curdistance)
                {
                    rnt = true;
                    //if (isLittleGuy)
                    //    Mogo.Util.Debug.LogError("IsInSkillRange:true" + distance + " " + entityRadius + " " + curdistance);
                }
                else
                {
                    rnt = false;
                    //if (isLittleGuy)
                    //    Mogo.Util.Debug.LogError("IsInSkillRange:false" + distance + " " + entityRadius + " " + curdistance);
                }
            }
            return rnt;
        }
