...
namespace Mogo.Game
{
    public partial class EntityMyself
    {
        ...
        {
			//Debug.Log("SanctuaryDefenseTimeResp()" + nextTimeSec + ":" + canEnterTime);
        ...
        {
			Debug.Log("OnRankingListUpdateResp()" + rank.ToString());
        ...
        {
			Debug.Log("OnSanctuaryDefenseRewardResp()" + reward.ToString());
        ...
        {
			//Debug.Log("UseItemResp()" + errorCode);
        ...
        {
            //Debug.Log("dbid = " + dbid + " name = " + name + "level = " + level);
        ...
        {
			//Debug.Log("get_reward_recharge_Resp()");
        ...
        {
            //Debug.Log("get_done_recharge_Resp");
        ...
        {
			//Debug.Log("get_reward_Resp() " + errorCode);
        ...
        {
            //Debug.Log("get_reward_login_Resp");
        ...
        {
            //Debug.Log("get_reward_achievement_Resp");
        ...
        {
			//Debug.Log("get_achievement_Resp()");
        ...
        public void ChargeSkillResp(uint dbid, byte flag)
        {
            if (dbid == this.ID)
            {
                Debug.Log("RPCResp ChargeSkillResp: myself");
                return;
            }
            EntityPlayer entity = (EntityPlayer)MogoWorld.GetEntity(dbid);
            if (entity == null)
            {
                Debug.LogError("RPCResp PowerUp args error : not found entity");
                return;
            }
            if (flag > 0)
            {
            ...
            switch (code)
            {
                ...
                case 1:
                    {//不存在下一级
                        ...
                        Debug.LogError("skillupresp code " + code);
                        break;
                    }
                case 2:
                    {//没有技能ID
        ...
        private void CastSkillResp(UInt32 id, Int16 x, Int16 z, byte face, UInt16 skillID)
        {
            ...
            {
                Debug.Log("skill player not exist " + id);
                return;
            }
        ...
        new UnityEngine.Vector2(entity.Transform.position.x, entity.Transform.position.z));
            if (dis > 2.0f)
            {
                //Debug.LogError("distance(server xy , client xy) too far, need correction:" + dis);
        ...
        {
            //Debug.LogError("SkillBuffResp:" + targetId + ",buffId:" + buffId);
        ...
        private void JewlMailResp(TDBID mailDBId)
        {
            ...
            if (rntFlag)
            {
                //Debug.LogError("Get mailDBId  mailGridId SUCCEEDED " + mailGridId + " mailDBID:" + mailDBId);
                TipManager.Instance.AddMailHasJewelTip(mailGridId);
            }
            else
            {
                //Debug.LogError("Get mailDBId  mailGridId FAILED " + mailGridId + " mailDBID:" + mailDBId); 
