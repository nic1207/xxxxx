...
public class SfxHandler : MonoBehaviour
{
    ...
    //    //{
    //    //    //Debug.Log(item.transform.parent.name + "......................" + item.name);
    ...
    public void HandleFx(int id, Transform target = null, Action<GameObject, int> action = null, string bone_path = "")
    {
        if (FXData.dataMap.ContainsKey(id))
        {
            ...
            Debug.LogWarning(string.Format("Can not find fxData {0}", id));
		}
    }
    ...
    private void OnChangeWeapon(GameObject go)
    {
        if (this && go)
        {
            ...
            if (wt)
            {
                //Debug.Log("MeleeWeaponTrail: " + go.name);
                m_weaponTrailsDic[wt.transform.parent.name] = wt;
            }
            else
            {
                //Debug.Log("No MeleeWeaponTrail: " + go.name);
    ...
    private void GetMaterials()
    {
        ...
        if (randers.Count != 0)
        {
            //Debug.LogWarning("randers.Count: " + randers.Count);
    ...
    private void SetMatShader(Shader shader)
    {
        if (m_mat != null && shader)
            ...
            {
                //Debug.LogError("SetMatShader item: " + item.name);
                item.shader = shader;
            }
    }
