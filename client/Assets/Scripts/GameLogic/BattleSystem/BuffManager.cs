...
public class BuffManager
{
    ...
    public void HandleBuff(ushort buffId, byte isAdd, uint time)
    {
        ...
        {
            Debug.Log("can find the buffId:" + buffId);
            Debug.LogError("can find the buffId:" + buffId);
            return;
        }
        ...
        if (isAdd == 1) // 添加Buff
        {
            Debug.Log("add buff fx:" + buff.sfx + ",time:" + time);
        ...
        else // 删除Buff
        {
            ...
            if (buff.notifyEvent == 1)
            {
                ...
                if (m_theOwner == MogoWorld.thePlayer)
                {
                    Debug.Log("Remove Buff : id =" + buff.id);
            ...
            {
                Debug.Log("delete buff fx:" + buff.sfx);
                if (m_theOwner.sfxHandler)
                {
