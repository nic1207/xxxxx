...
public class OperationSystem : IEventManager
{
	...
	{
		//Debug.Log("OperationSystem Charge()");
		RpcCharge();
	}

	protected void ChargeGetRewardGot()
	{
		//Debug.Log("OperationSystem ChargeGetReward()");
	...
	{
		//Debug.Log("OperationSystem EventGetReward()");
		RpcEventGetReward(id);
	}

	protected void EventShareToGetDiamond()
	{
		//Debug.Log("OperationSystem EventShareToGetDiamond()");
		RpcEventShareToGetDiamond();
	}

	protected void LogInGetReward(int id)
	{
		//Debug.Log("OperationSystem LogInGetReward()");

		//if (RewardLoginData.dataMap[id].days != theOwner.login_days)
		//{
		//    if (RewardLoginData.dataMap[id].days == theOwner.login_days - 1)
		//    {
		//        Debug.Log("Maybe You Just Pass 0:00... Check Your Mail");
		//        return;
		//    }
		//    else
		//    {
		//        Debug.Log("OperationSystem LogInGetReward Days: " + RewardLoginData.dataMap[id].days + " The Owner Days" + theOwner.login_days);
		//        Debug.Log("If you see the debug here, means that the login_days is 0");
		//        Debug.Log("Check you def file and property synchronization");
		//        return;
		//    }
		//}
	...
	{
		//Debug.Log("OperationSystem GetChargeRewardMessage()");
		SetChargeRewardMessage();
	}

	protected void GetActivityMessage()
	{
		//Debug.Log("OperationSystem GetActivityMessage()");
		// ShowEventDoing();
		SetActivityMessage();
		// RpcGetActivityID();
	}
	...
	{
		//Debug.Log("OperationSystem GetLoginMessage()");
	...
	{
		//Debug.Log("OperationSystem GetAchievementMessage()");
	...
	{
		//Debug.Log("OperationSystem EventTimesUp()");
		RpcLeaveEvent(eventID);
	}

	protected void GetLoginMarket()
	{
		//Debug.Log("OperationSystem GetLoginMarket()");
	...
	{
		//Debug.Log("RpcEventGetReward(" + id+")");
	...
	{
		//Debug.Log("RpcEventShareToGetDiamond()");
		// theOwner.RpcCall("");
	...
	{
		//Debug.Log("RpcGetAllActivityEnable()");
		theOwner.RpcCall("EventOpenList");
	}

	public void RpcGetAllActivityDoing()
	{
		//Debug.Log("RpcGetAllActivityDoing()");
		theOwner.RpcCall("get_event_ing");
	}

	public void RpcAddActivity(int id)
	{
		//Debug.Log("RpcAddActivity(" + id+")");
	...
	{
		//Debug.Log("RpcGetAchievementID(" + id+")");
	...
	{
		//Debug.Log("RpcAchievemenShareToGetDiamond(" + id+")");
		// Temp
		theOwner.RpcCall("get_reward_achievement", (UInt16)id);
	...
	{
		//Debug.Log("RpcUpdateMarketCheck(" + version+")");
	...
	public void RpcLoginMarketBuy()
	{
		...
		{
			//Debug.Log("RpcUpdateMarket Time > 86400");
			...
			if (LoginMarketData.dataMap.ContainsKey(dataID))
			{
				...
				//Debug.Log("RpcUpdateMarket Version: " + data.version + " ItemID: " + data.itemId + " PriceType: " + data.priceType + " Price: " + data.price + " Date: " + MogoTime.Instance.GetCurrentDateTime().ToString());
	...
	{
		//Debug.Log("OnEventGetReward(" + theID+")");
		int id = (int)theID;
	...
	{
		//Debug.Log("OnAchievemenShareToGetDiamond()");
	...
	{
		Debug.Log("chargeSum(" + theOwner.chargeSum+")");
		...
		if (theOwner.chargeSum > 0)
		{
			...
			foreach (var data in RewardRechargeData.dataMap)
			{
				...
				{
					Debug.Log("SetChargeRewardMessage(" + i+")");
	...
	{
		// to do 
		//Debug.Log("SetEventGetReward(" + id+")");
	...
	{
		//Debug.Log("HandleActivityEnableLuaTable()");

		object obj;
		if (Utils.ParseLuaTable(luaTable, typeof(List<int>), out obj))
		{
			//Debug.Log("HandleActivityEnableLuaTable()" + Mogo.RPC.Utils.PackLuaTable(luaTable));
			eventEnable.Clear();
			eventEnable = obj as List<int>;
			//Debug.Log("HandleActivityEnableLuaTable Format Size: " + eventEnable.Count);

			SetActivityMessage();
		}
		else
		{
			Debug.LogError("HandleActivityEnableLuaTable Failed");
		}
	}

	protected void HandleActivityDoingLuaTable(LuaTable luaTable, LuaTable luaTableToDo)
	{
		//Debug.Log("HandleActivityDoingLuaTable()");

		object obj;
		object objToDo;
		//if (Utils.ParseLuaTable(luaTable, typeof(Dictionary<int, EventMessageData>), out obj))
		//{
		//    Debug.Log("HandleActivityDoingLuaTable" + Mogo.RPC.Utils.PackLuaTable(luaTable));
		//    eventDoing.Clear();
		//    eventDoing = obj as Dictionary<int, EventMessageData>;

		//    if (eventDoing == null)
		//        Debug.Log("HandleActivityDoingLuaTable Format Size: " + "eventDoing.Count == null");
		//    else
		//        Debug.Log("HandleActivityDoingLuaTable Format Size: " + eventDoing.Count);
		...
		{
			//Mogo.Util.Debug.LogError("HandleActivityDoingLuaTable" + Mogo.RPC.Utils.PackLuaTable(luaTable));
			//Mogo.Util.Debug.LogError("HandleActivityToDoLuaTable" + Mogo.RPC.Utils.PackLuaTable(luaTableToDo));
			...
			if (eventDoing == null) {
				Debug.LogError ("HandleActivityDoingLuaTable Format Size: " + "eventDoing.Count == null");
			} else {
				//Debug.Log ("HandleActivityDoingLuaTable() eventDoing.Count=" + eventDoing.Count);
			}
		...
		{
			Debug.LogError("HandleActivityDoingLuaTable Failed");
		}
	}
	...
	protected void SetActivityMessage()
	{
		...
		foreach (int enableID in eventEnable)
		{
			...
			{
				Debug.Log("Activity " + enableID + " Not Exist!");
				continue;
			}
	...
	{
		Debug.Log("CheckEventOpen()");

		if (entity.ID != theOwner.ID)
		{
			Debug.Log("CheckEventOpen Not EntityMyself!");
			return;
		}
			...
			if (EventData.dataMap.ContainsKey(listeningID))
			{
				...
				foreach (var condition in data.conditions)
				{
					switch (condition.Key)
					{
						case 1:
							// Rpc Pass Judge
							Debug.Log("CheckEventOpen Rpc Pass Judge");
							if (theOwner.level >= condition.Value)
								conditionCount++;
							break;
				...
				Debug.Log("CheckEventOpen Rpc Pass Judge: conditionCount: " + conditionCount);
				Debug.Log("CheckEventOpen Rpc Pass Judge: data.conditions.Count: " + data.conditions.Count);
				if (conditionCount == data.conditions.Count)
				{
					Debug.Log("CheckEventOpen Rpc Pass Judge: conditionCount == data.conditions.Count");
					RpcAddActivity(listeningID);
				}
			}
		}
	...
	{
		//Debug.Log("CheckLoginFirstShow: " + loginFirstShow + " " + loginRewardHasGot);
		...
		{
			//Debug.Log("CheckLoginFirstShow In");
	...
	protected void SetLoginMessage()
	{
		...
		foreach (var data in RewardLoginData.dataMap)
		{
			...
			{
				Debug.Log("RewardLoginData level wrong, id: " + data.Key);
				continue;
			}
	...
	{
		//Debug.Log("HandleAchievementLuaTable()");

		object obj;
		//if (Utils.ParseLuaTable(luaTable, typeof(List<AchievementMessageData>), out obj))
		//{
		//    Debug.Log("achievementDoing" + Mogo.RPC.Utils.PackLuaTable(luaTable));
		//    achievementDoing.Clear();
		//    achievementDoing = obj as List<AchievementMessageData>;
		//    Debug.Log("HandleAchievementLuaTable Format Size: " + achievementDoing.Count);
		//}
		//else
		//{
		...
		{
			Debug.LogError("HandleAchievementLuaTable Failed");
		}
	...
	{
		//Debug.Log("UpdateAchievementLuaTable()");
		...
		//    Debug.Log("UpdateAchievementLuaTable Format Size: " + motifyList[0].aid + " " + motifyList[0].level + " " + motifyList[0].reward_level + " " + motifyList[0].cur_num);
		...
		{
			Debug.Log("HandleAchievementLuaTable Failed");
		}
	...
	{
		//Debug.Log("SetAchievementMessage()");
		...
		foreach (var item in achievementDoing)
		{
			...
			//Debug.Log("Achievement: " + item.aid + " " + item.level + " " + item.reward_level + " " + item.cur_num);
			...
			if (RewardAchievementData.dataMap.ContainsKey(id))
			{
				if (isRefreshUI)
				{
					Debug.Log("id: " + id + " item.cur_num: " + item.cur_num + " isShared: " + isShared + " isFinished: " + isFinished);
			...
			{
				Debug.Log("Achievement Id = " + id + " Not Exist");
			}
		}
	...
	{
		Debug.Log("SetAchievemenShareToGetDiamond( " + theID+")");
	...
	public void HandleErrorMsg(int errorCode, string from, int arg = 0)
	{
		...
		{
			case 1:
				Debug.Log("error_code_event_ing " + arg + " ");
				// sb.Append("error_code_event_ing " + arg + " ");
				break;
			case 2:
				Debug.Log("error_code_event_not_in_time " + arg + " ");
				// sb.Append("error_code_event_not_in_time");
				break;
			case 3:
				Debug.Log("error_code_event_beyond_count " + arg + " ");
				// sb.Append("error_code_event_beyond_count");
				break;
			case 4:
				Debug.Log("error_code_event_closed " + arg + " ");
				// sb.Append("error_code_event_closed");
				break;
			case 5:
				Debug.Log("error_code_event_done " + arg + " ");
				// sb.Append("error_code_event_done");
				break;
			case 6:
				Debug.Log("error_code_event_no_begin " + arg + " ");
				// sb.Append("error_code_event_no_begin");
				break;
			case 7:
				Debug.Log("error_code_event_not_finish " + arg + " ");
				// sb.Append("error_code_event_not_finish");
				break;
			case 8:
				Debug.Log("error_code_event_rewarded ");
