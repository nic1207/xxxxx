...
public class CalculateDamage
{
    ...
    public static List<int> CacuDamage(int hitActionID, uint attackerID, uint victimID)
    {
        ...
        {
            Debug.LogError("CacuDamage Error. eid not exist.");
            result.Add(-1);
            result.Add(0);
            return result;
        }
        ...
        if (isDebugInfo)
        {
            //Debug.LogError("dmgAll:" + dmgAll + " dmgNormal:" + dmgNormal + " atk:" + atk + " skillMul:" + skillMul + " skillAdd:" + skillAdd + " dmgCorrect:" + dmgCorrect + " damageReduce:" + damageReduce + " result:" + (Math.Ceiling(dmgAll / temp * RandomHelper.GetRandomFloat(0.90f, 1.10f))));
        }
        return result;
    }
