...
public class MultiPlatformControllerGear : GearParent
{
    ...
    void Start()
    {
        ...
        {
            Debug.Log("Length is not matching width * height!");
            enabled = false;
        }

        if (width < 2 || height < 2)
        {
            Debug.Log("Width and height must be bigger than 2!");
            enabled = false;
        }
    ...
    private void OnMoveDown(bool horizontal, int breachResult)
    {
        ...
        Debug.Log(count + " all curPlatform");
        Debug.Log(curPlatform + " curPlatform");
