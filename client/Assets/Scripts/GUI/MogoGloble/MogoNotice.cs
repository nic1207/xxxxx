...
public class MogoNotice : MonoBehaviour
{
    ...
    private void FillFullNameData(Transform rootTransform)
    {
        for (int i = 0; i < rootTransform.GetChildCount(); ++i)
        {
            ...
            {
                Debug.Log("rootTransform.GetChild(i):" + rootTransform.GetChild(i).name);
            }
    ...
    public IEnumerator ShowNotice(List<Notice> list)
    {
        ...
        {
            Debug.Log("m_contentLabel is Loading!");
            yield return null;
        }
        isShow = true;
        Reset();
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            AssetCacheMgr.GetUIInstance(TITLE_PREFAB, (str, id, obj) =>
            {
                ...
                //Debug.Log("list[index].title:" + list[count1].title);
            ...
            //����߶�
            if (m_contentLabel == null) Debug.LogError("m_contentLabel == null");
            if (list.Count <= index) Debug.LogError("list[index] == null");
