...
public class TestAssetbundle : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //{
        //    Debug.Log(item.Key + " " + item.Value.Path);
        //}
        ...
        foreach (var build in buildInfo)
        {
            if (build.name == "UI")
            {
                ...
                foreach (var item in dirs)
                {
                    ...
                    //Debug.Log(Utils.GetFileNameWithoutExtention(defFiles[0], '\\'));
                    ...
                    if (!m_dicMap.ContainsKey(fileName))
                        ...
                        Debug.Log("fileName exist: " + fileName);
                    BuildMogoAssetBundleInfo(folder);
                }
            }
        }
        Debug.Log("m_allRecources.Count: " + m_allRecources.Count);
        Debug.Log("m_assetBundleDic.Count: " + m_assetBundleDic.Count);
    }
    ...
    private void BuildMogoAssetBundleInfo(string folder)
    {
        ...
        foreach (var item in defFiles)
        {
            ...
            foreach (SecurityElement se in xml.Children)
            {
                ...
                //Debug.LogWarning(asset.Path + " " + key);
    ...
    private AssetBundleInfo GetAssetBundleInfo(SecurityElement se, string parentFolder)
    {
        ...
        {
            //Debug.LogWarning("Same resource: " + path);
    ...
    private void LoadAssetBundleInfo(string prefab, System.Action<AssetBundleInfo> loaded)
    {
        ...
        if (flag)
        {
            ...
            {//资源已加载，直接回调
                //Debug.Log("resource loaded: " + assetBundleInfo.Path);
            ...
            else
            {
                ...
                Debug.Log("FileCount: " + loadList.LoadList.Count);
                if (m_isLoading)
                {
                    //Debug.Log("m_resourceLoadQueue Enqueue: " + assetBundleInfo.Path + m_resourceLoadQueue.Count);
    ...
    private HashSet<string> GetAllFile(AssetBundleInfo info)
    {
        ...
        if (info.Asset == null)
        {
            foreach (var item in info.SubResource)
            {
                ...
                {
                    if (!list.Add(set))
                        Debug.Log("exist 1: " + set);
                }
            }
            if (!list.Add(info.Path))
                Debug.Log("exist 2: " + info.Path);
        }
        return list;
    }
    ...
    private void ContinueLoad()
    {
        lock (m_resourceLoadQueueLocker)
        {
            ...
            {
                //Debug.Log("m_resourceLoadQueue Dequeue: " + m_resourceLoadQueue.Count);
    ...
    public class AssetBundleInfo
    {
        ...
        public Object GetInstance()
        {
            ...
            {
                Debug.LogError("AssetBundle get instance: '" + Path + "' error: " + ex.Message);
                return null;
            }
        }
