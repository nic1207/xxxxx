...
namespace mogo
{
    ...
    void CEntityBase::AddCellMailbox(int32_t n, uint16_t nCellSvrId)
    {
        ...
        if(m_bHasClient)
        {
LogInfo("CEntityBase::AddCellMailbox", "--------------- MSGID_CELLAPP_PICKLE_AOI_ENTITIES --------id=%d", m_id);
...
LogDebug("CEntityBase::lNotifyClientToAttach", "---------- lNotifyClientToAttach ------id=%d;Account=%s", m_id, pszAccount);
...
LogDebug("CEntityBase::lNotifyClientToAttach", "---------- MSGID_LOGINAPP_NOTIFY_CLIENT_TO_ATTACH ------pszAccount=%s; MailboxId=%d; key.c_str()=%s", pszAccount, m_mymb.m_nServerMailboxId, key.c_str());
        }
