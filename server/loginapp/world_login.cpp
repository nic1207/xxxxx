    ...
    int CWorldLogin::FromRpcCall(CPluto& u)
    {
        ...
        switch(msg_id)
        {
            case MSGID_LOGINAPP_CHECK:
            {
                ...
                //版本校验
                // nRet = VersionCheck(p);
                LogDebug("CWorldLogin::logincheck", "xxxxxx");
		nRet = 0;//loveomg version check pass
                break;
            }
            case MSGID_LOGINAPP_LOGIN:
            {
                AddClientFdToVObjectList(u.GetMailbox()->GetFd(), p);
                nRet = AccountLogin(p);
                LogDebug("CWorldLogin::FromRpcCall", "xxxxx-----------------MSGID_LOGINAPP_LOGIN--------- nRet=%d", nRet);
                break;
            }
            case MSGID_LOGINAPP_WEBLOGIN:
            {
                AddClientFdToVObjectList(u.GetMailbox()->GetFd(), p);
                LogDebug("CWorldLogin::FromRpcCall", "-----------------MSGID_LOGINAPP_WEBLOGIN--------- nRet=%d", nRet);
            ...
            case MSGID_LOGINAPP_SELECT_ACCOUNT_CALLBACK:
            {
                ...
                LogDebug("CWorldLogin::FromRpcCall", "-----------------MSGID_LOGINAPP_SELECT_ACCOUNT_CALLBACK--------- nRet=%d", nRet);
                break;
            }
            case MSGID_LOGINAPP_NOTIFY_CLIENT_TO_ATTACH:
            {
                nRet = NotifyClientToAttach(p);
                LogDebug("CWorldLogin::FromRpcCall", "-------------------NotifyClientToAttach------- nRet=%d", nRet);
                break;
            }
            case MSGID_LOGINAPP_NOTIFY_CLIENT_MULTILOGIN:
            {
                nRet = NotifyClientMultiLogin(p);
		LogDebug("CWorldLogin::FromRpcCall", "-------------------MSGID_LOGINAPP_NOTIFY_CLIENT_MULTILOGIN------- nRet=%d", nRet);
                break;
            }
            case MSGID_LOGINAPP_MODIFY_LOGIN_FLAG:
            {
                nRet = ModifyLoginFlag(p);
LogDebug("CWorldLogin::FromRpcCall", "-------------------MSGID_LOGINAPP_MODIFY_LOGIN_FLAG------- nRet=%d", nRet);
                break;
            }
            case MSGID_LOGINAPP_FORBIDLOGIN:
            {
                nRet = ForbidLogin(p);
LogDebug("CWorldLogin::FromRpcCall", "-------------------MSGID_LOGINAPP_FORBIDLOGIN------- nRet=%d", nRet);
                break;
            }
            case MSGID_LOGINAPP_FORBID_IP_UNTIL_TIME:
            {
                nRet = ForbidLoginByIp(p);
LogDebug("CWorldLogin::FromRpcCall", "-------------------MSGID_LOGINAPP_FORBID_IP_UNTIL_TIME------- nRet=%d", nRet);
                break;
            }
...
LogDebug("CWorldLogin::FromRpcCall", "-------------------MSGID_LOGINAPP_FORBID_ACCOUNT_UNTIL_TIME------- nRet=%d", nRet);
                break;
            }
...
LogDebug("CWorldLogin::FromRpcCall", "-------------------MSGID_ALLAPP_SHUTDOWN_SERVER------- nRet=%d", nRet);
                break;
            }
            case MSGID_LOGINAPP_MODIFY_ONLINE_COUNT:
            {
                nRet = ModifyOnlineCount(p);
LogDebug("CWorldLogin::FromRpcCall", "-------------------MSGID_LOGINAPP_MODIFY_ONLINE_COUNT------- nRet=%d", nRet);
                break;
            }
...
LogDebug("CWorldLogin::FromRpcCall", "-------------------MSGID_LOGINAPP_LOGIN_VERIFY_CALLBACK------- nRet=%d", nRet);
				break;
			}
#endif
            default:
	...
	{
LogDebug("CWorldLogin::_AccountLoginImpl", "--------------_AccountLoginImpl------");

/*
		if(p->size() != 3)		//loveomg force login
		{
			return -1;
		}
*/

LogDebug("CWorldLogin::_AccountLoginImpl", "-----------_AccountLoginImpl---p->size()=%d", p->size());
LogDebug("CWorldLogin::_AccountLoginImpl", "-----------_AccountLoginImpl;p1=%s;p2=%s;p3=%s", VOBJECT_GET_I32((*p)[0]), VOBJECT_GET_I32((*p)[1]), VOBJECT_GET_I32((*p)[2]));
...
*/	//loveomg force login end -----------------------------------------------
	...
	//int32_t nFd = VOBJECT_GET_I32((*p)[2]);
        ...
        if(bLoginFirst)
        {
            ...
            if(mb2)
            {
LogInfo("CWorldLogin::account_login", "-------------- MSGID_DBMGR_SELECT_ACCOUNT----;pszAccount=%s; pszPasswd=%s; nFd=%d", pszAccount, pszPasswd, nFd);
        ...
        else
        {
            ...
            if(mb2)
            {
LogInfo("CWorldLogin::account_login", "-------------- MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME_ANYWHERE----;pszAccount=%s", pszAccount);
                //平台认证后的登陆直接进了
    ...
    {
LogInfo("CWorldLogin::select_account_callback", "----------------- SelectAccountCallback 1 ------");

       if(p->size() != 3)
        {
            return -1;
        }
LogInfo("CWorldLogin::select_account_callback", "----------------- SelectAccountCallback 2 ------");
...
LogInfo("CWorldLogin::select_account_callback", "----------------- ENUM_LOGIN_RET_ACCOUNT_PASSWD_NOMATCH ------");
            //账号错误
...
LogInfo("CWorldLogin::select_account_callback", "----------------- MAILBOX_CLIENT_AUTHZ ------");
            }
            (*pu).Encode(MSGID_CLIENT_LOGIN_RESP) << (uint8_t)ENUM_LOGIN_SUCCESS << EndPluto;

LogInfo("CWorldLogin::select_account_callback", "----------------- ENUM_LOGIN_SUCCESS ------");
            //创建account
            CMailBox* mb2 = GetServerMailbox(SERVER_BASEAPPMGR);
            if(mb2)
            {
LogInfo("CWorldLogin::select_account_callback", "----------------- MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME_ANYWHERE 1 ------");
                //标记1表示如果数据库中不存在,也要创建一个entity
                mb2->RpcCall(GetRpcUtil(), MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME_ANYWHERE, (uint8_t)1, "Account", pszAccount);
LogInfo("CWorldLogin::select_account_callback", "-------dbmgr ----- MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME_ANYWHERE 2 ------");
            }
        }
    ...
    int CWorldLogin::NotifyClientToAttach(T_VECTOR_OBJECT* p)
    {
        ...
        //const char* pszKey = "e5a2d6e9b88cff1622d5b58a25f71496";
        const char* pszKey = VOBJECT_GET_STR((*p)[2]);
        
LogDebug("NotifyClientToAttach", "------------- NotifyClientToAttach ------------ pszAccount=%s;baseapp_id=%d;pszKey=%s", pszAccount, baseapp_id, pszKey);
...
LogDebug("NotifyClientToAttach", "----------------NotifyClientToAttach-------- MSGID_CLIENT_NOTIFY_ATTACH_BASEAPP=%d; GetServerName=%s; GetServerPort=%d; baseapp_id=%d", MSGID_CLIENT_NOTIFY_ATTACH_BASEAPP, smb->GetServerName().c_str(), smb->GetServerPort(), baseapp_id);
