...
LogDebug("CWorldMgrD::FromRpcCall", "-------------- CheckClientRpc -----------");
            return -1;
        }
...
LogDebug("CWorldMgrD::FromRpcCall", "-------------- T_VECTOR_OBJECT -----------");
            return -1;
        }
...
LogDebug("CWorldMgrD::FromRpcCall", "-------------- GetDecodeErrIdx -----------");
            return -2;
        }
...
LogDebug("CWorldMgrD::FromRpcCall", "-------------- MSGID_BASEAPPMGR_REGISTERGLOBALLY ----------- nRet=%d", nRet);
                break;
            }
            case MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME_ANYWHERE:
            {
                nRet = CreateBaseFromDbByName(p);
LogDebug("CWorldMgrD::FromRpcCall", "-------------- MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME_ANYWHERE ----------- nRet=%d", nRet);
                break;
            }
            case MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME:
            {
                nRet = CreateBaseFromDbByName(p);
LogDebug("CWorldMgrD::FromRpcCall", "-------------- MSGID_BASEAPPMGR_CREATEBASE_FROM_NAME ----------- nRet=%d", nRet);
                break;
            }
            case MSGID_BASEAPPMGR_CREATE_CELL_IN_NEW_SPACE:
            {
                nRet = CreateCellInNewSpace(p);
//LogDebug("CWorldMgrD::FromRpcCall", "-------------- MSGID_BASEAPPMGR_CREATE_CELL_IN_NEW_SPACE ----------- nRet=%d", nRet);
                break;
            }
            case MSGID_BASEAPPMGR_CREATEBASE_ANYWHERE:
            {
                nRet = CreateBaseAnywhere(p);
//LogDebug("CWorldMgrD::FromRpcCall", "-------------- MSGID_BASEAPPMGR_CREATEBASE_ANYWHERE ----------- nRet=%d", nRet);
                break;
            }
            case MSGID_BASEAPPMGR_SHUTDOWN_SERVERS:
            {
                nRet = ShutdownAllServers(p, u);
LogDebug("CWorldMgrD::FromRpcCall", "-------------- MSGID_BASEAPPMGR_SHUTDOWN_SERVERS ----------- nRet=%d", nRet);
                break;
            }
            case MSGID_BASEAPPMGR_ON_SERVER_SHUTDOWN:
            {
                nRet = OnServerShutdown(p);
LogDebug("CWorldMgrD::FromRpcCall", "-------------- MSGID_BASEAPPMGR_ON_SERVER_SHUTDOWN ----------- nRet=%d", nRet);
                break;
            }
        }
        ...
        if(nRet != 0)
        {
            LogWarning("from_rpc_call", "------------- CWMD ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
        }
    ...
    {
//LogDebug("CWorldMgrD::CreateBaseFromDbByName", "----------CreateBaseFromDbByName-- START -----------");
        if(p->size() < 3)
        {
//LogDebug("CWorldMgrD::CreateBaseFromDbByName", "----------CreateBaseFromDbByName-- ERROR --p->size()=%d", p->size());
            return -1;
        }
...
//LogDebug("CWorldMgrD::CreateBaseFromDbByName", "----------CreateBaseFromDbByName-- __TEST_LOGIN -----------");
...
#else
//LogDebug("CWorldMgrD::CreateBaseFromDbByName", "----------CreateBaseFromDbByName-- NORMAL_LOGIN -----------");
        CMailBox* mb = GetServerMailbox(SERVER_DBMGR);
        if(mb)
        {
LogDebug("CWorldMgrD::CreateBaseFromDbByName", "------------ MSGID_DBMGR_CREATEBASE_FROM_NAME -----------");
