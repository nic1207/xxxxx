...
namespace mogo
{
    ...
    int CWorldDbmgr::FromRpcCall(CPluto& u, CDbOper& db)
    {
        ...
        switch(msg_id)
        {
            ...
            {
                nRet = InsertDB(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_INSERT_ENTITY ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_SELECT_ENTITY:
            {
                nRet = LookupEntityByDbId(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_SELECT_ENTITY ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_UPDATE_ENTITY:
            {
                nRet = UpdateEntity(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_UPDATE_ENTITY ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_UPDATE_ENTITY_REDIS:
            {
                nRet = UpdateEntityToRedis(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_UPDATE_ENTITY_REDIS ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_SELECT_ACCOUNT:
            {
                nRet = SelectAccount(p, db);
LogWarning("from_rpc_call", "------------- MSGID_DBMGR_SELECT_ACCOUNT ------- rmsg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_RAW_MODIFY_NORESP:
            {
                nRet = QueryModifyNoResp(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_RAW_MODIFY_NORESP ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_CREATEBASE_FROM_NAME:
            {
                nRet = LookupEntityByName(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_CREATEBASE_FROM_NAME ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_LOAD_ALL_AVATAR:
            {
                nRet = LoadAllAvatars(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_LOAD_ALL_AVATAR ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_LOAD_ENTITIES_OF_TYPE:
            {
                nRet = LoadEntitiesOfType(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_LOAD_ENTITIES_OF_TYPE ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_SHUTDOWN_SERVER:
            {
                nRet = ShutdownServer(p);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_SHUTDOWN_SERVER ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_REDIS_HASH_LOAD:
            {
                nRet = RedisHashLoad(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_REDIS_HASH_LOAD ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_REDIS_HASH_SET:
            {
                nRet = RedisHashSet(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_REDIS_HASH_SET ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_REDIS_HASH_DEL:
            {
                nRet = RedisHashDel(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_REDIS_HASH_DEL ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_DEL_ACCOUNT_CACHE:
            {
                nRet = DestroyAccountCache(p);
 //LogWarning("from_rpc_call", "------------- MSGID_DBMGR_DEL_ACCOUNT_CACHE ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
               break;
            }
...
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_UPDATE_ITEMS ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_LOADING_ITEMS:
            {
                nRet = LoadingArrayItemsToInventory(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_LOADING_ITEMS ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_INCREMENTAL_UPDATE_ITEMS:
            {
                nRet = IncremantalUpdateItems(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_INCREMENTAL_UPDATE_ITEMS ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
...
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_TABLE_SELECT ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_UPDATE_BATCH:
            {
                nRet = TableUpdateBatch(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_UPDATE_BATCH ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_TABLE_INSERT:
            {
                nRet = TableInsert(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_TABLE_INSERT ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_TABLE_EXCUTE:
            {
                nRet = TableExcute(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_TABLE_EXCUTE ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_TABLE2_SELECT:
            {
                nRet = Table2Select(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_TABLE2_SELECT ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            case MSGID_DBMGR_TABLE2_INSERT:
            {
                nRet = Table2Insert(p, db);
 //LogWarning("from_rpc_call", "------------- MSGID_DBMGR_TABLE2_INSERT ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
               break;
            }
            case MSGID_DBMGR_TABLE2_EXCUTE:
            {
                nRet = Table2Excute(p, db);
//LogWarning("from_rpc_call", "------------- MSGID_DBMGR_TABLE2_EXCUTE ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
                break;
            }
            default:
            {
        ...
        if(nRet != 0)
        {
            //LogWarning("from_rpc_call", "------------- DBMGR ------- rpc error.msg_id=%d;ret=%d\n", msg_id, nRet);
        }
    ...
    {
LogDebug("CWorldDbmgr::LookupEntityByName", "----------LookupEntityByName---START-------------------");
        if(p->size() != 4)
        {
            return -1;
        }
        ...
        if(pLookup != NULL)
        {
LogInfo("CWorldDbmgr::LookEntityByName", "-------------LookupEntityByName---------entity exists;key=%s;eid=%d;baseapp=%d", pszKey, pLookup->eid, pLookup->sid);
...
LogDebug("CWorldDbmgr::LookupEntityByName", "---------------LookupEntityByName-----u.GenLen()=%d", u->GetLen());
        ...
        if(db.LookupEntityByName(nCreateFlag, pszEntityName, strKey, new_eid, *u, nBaseappId, strErr) != 0)
        {
            delete u;
            //cout << strErr << endl;
LogWarning("LookEntityByName_err", "-----------------LookEntityByName_err---- %s", strErr.c_str());
            return -2;
        }
        ...
        if(mb)
        {
            ...
            LogDebug("CWorldDbmgr::LookupEntityByName", "---------NON WIN32-------u.GenLen()=%d", u->GetLen());
...
LogDebug("CWorldDbmgr::SelectAccount", "-----------nServerId=%d; nFd=%d", nServerId, nFd);


//        uint16_t nServerId = 1;			//loveomg login pass
//        int32_t nFd = 6;
//        const char* s1 = "111111@test";
        const char* s2 = "111111@test";



LogDebug("CWorldDbmgr::SelectAccount", "--------------- s1=%s; s2=%s; nServerId=%d; nFd=%d", s1, s2, nServerId, nFd);
