...
//#endif
	...
	{
LogDebug("CDbOper::SelectAccount", "-------------- SelectAccount() Sponser --------------");
		if(!m_bConnectDb)
		{
			strErr.assign("db not connected");
			return 0;
    ...
    {
LogDebug("CDbOper::SelectAccount", "-------------- SelectAccount() --------------");

        if(!m_bConnectDb)
        {
            strErr.assign("db not connected");
LogDebug("CDbOper::SelectAccount", "-------------- db not connected --------------");
            return 0;
        }
...
LogDebug("CDbOper::SelectAccount", "------------- SelectAccount -------- szSql=%s", szSql);

        int nRet = mysql_real_query(m_mysql, szSql, (unsigned long)strlen(szSql));
        if(nRet != 0)
        {
            strErr.assign(mysql_error(m_mysql));
LogDebug("CDbOper::SelectAccount", "---------------- mysql_error ----------------");
            return 0;
        }
